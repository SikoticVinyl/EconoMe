...
function UpdateBudget({ handleSave }) {
    // State for the static savings goal
    const [newSavingsGoal, setNewSavingsGoal] = useState('');
    const [submittedSavingsGoal, setSubmittedSavingsGoal] = useState('');
    const [incomeTransactionsData, setIncomeTransactionsData] = useState([]);
    const [expenseTransactionsData, setExpenseTransactionsData] = useState([]);
    const [flexibleExpenseTransactionsData, setFlexibleExpenseTransactionsData] = useState([]);
    ...
    return (
        <div className="flex flex-col h-screen justify-center items-center relative" style={{
            backgroundImage: `url(/Moneybg.jpg)`,
            backgroundSize: '100% 100%'
        }}>
            <div className="bg-[#5549ad] bg-opacity-90 text-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
                    ...
                    {incomeTransactions.map((transaction, index) => (
                        <div key={index} className="flex items-center justify-between">
                            ...
                            <input
                                ...
                                className="text-right px-2 rounded text-lg font-bold ml-2 w-20 cursor-pointer"
                                value={incomeTransactionsData[index] || ''}
                                onChange={e => {
                                    const newData = [...incomeTransactionsData];
                                    newData[index] = e.target.value;
                                    setIncomeTransactionsData(newData);
                                }}
                            />
                        </div>
                    ))}
                    ...
                    {expenseTransactions.map((transaction, index) => (
                        <div key={index} className="flex items-center justify-between">
                            ...
                            <input
                                placeholder={transaction.amount}
                                className="text-right px-2 rounded text-lg font-bold ml-2 w-20 cursor-pointer"
                                value={expenseTransactionsData[index] || ''}
                                onChange={e => {
                                    const newData = [...expenseTransactionsData];
                                    newData[index] = e.target.value;
                                    setExpenseTransactionsData(newData);
                                }}
                            />
                        </div>
                    ))}
                    ...
                    {flexibleExpenseTransactions.map((transaction, index) => (
                        <div key={index} className="flex items-center justify-between">
                            ...
                            <input
                                placeholder={transaction.amount}
                                className="text-right px-2 rounded text-lg font-bold ml-2 w-20 cursor-pointer"
                                value={flexibleExpenseTransactionsData[index] || ''}
                                onChange={e => {
                                    const newData = [...flexibleExpenseTransactionsData];
                                    newData[index] = e.target.value;
                                    setFlexibleExpenseTransactionsData(newData);
                                }}
                            />
                        </div>
                    ))}
